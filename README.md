...
Over Christmas I planned to complete the full ISL alphabet, polish the VR environment, and add user accounts, calibration, and data saving, but holidays meant the project stayed at an early prototype stage. The app ran on the Meta Quest 3 in passthrough, showing an ISL alphabet board and a feedback label, with hand tracking reliably recognising just three letters: A, V, and D.  

To make this prototype feel more like an actual application, I added a simple user account system using JSON files. I created a global script (`Global.gd`) to hold the current username and build per‑user save paths in Godot’s `user://` folder. I then built a 2D main menu scene (`MainMenu.tscn`) with a title, a name input field, and a “Start / Calibrate” button. The `MainMenu.gd` script reads the entered name, stores it in `Global.current_username`, saves an account file like `Name_account.json`, and then loads the existing VR scene (`main.tscn`).  

The next implementation will move this account creation into VR itself. I plan to design a 3D UI panel that appears inside the headset, along with a floating virtual keyboard so the user can type their name directly in VR using hand tracking. This will let the account creation feel fully integrated into the XR experience rather than relying on a 2D scene.  

Overall, the current changes mean the application now starts with a clear menu → account → VR flow, and user accounts are written to disk in JSON, laying the groundwork for later storing calibration data and progress.
